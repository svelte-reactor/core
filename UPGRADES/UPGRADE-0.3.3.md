...
v0.3.3 focuses on IndexedDB performance improvements and better collection support:
...
3. **Query Support** - Basic querying for collections (Preview)
4. **Test Optimization** - Faster test suite execution
...
// After: Connections are pooled automatically
const store1 = createReactor(state1, { plugins: [persist({ storage: 'indexedDB' })] });
const store2 = createReactor(state2, { plugins: [persist({ storage: 'indexedDB' })] });
// = 1 shared connection
```
...
v0.3.3 is fully backward compatible. All improvements are opt-in or automatic.
...
// Batch writes: enabled by default
persist({
  storage: 'indexedDB',
...
// Query support: opt-in (requires collections)
const results = await store.query('items', {
  where: { status: 'active' },
  limit: 10
});
```
